...
class Cart extends MY_Controller
{
    ...
    function getAddress($address)
    {
        ...
        try {
            $json = file_get_contents('https://maps.google.com/maps/api/geocode/json?address=' . $address . '&key=AIzaSyCDSWTa_Di55Iy7fgPgE9agEpf8-txNums');
    ...
    function save_order()
    {
        ...
        if (!empty($user_addr_det)) {
            ...
            $address = implode(' ', array($input_data['street'], $input_data['landmark'], $input_data['city'], $input_data['locality']));
            $array  = $this->getAddress($address);
            $input_data['latitude']  = round($array['lat'], 6);
            $input_data['longitude'] = round($array['long'], 6);
        }
    ...
    function order_success()
    {
        ...
        // $this->prepare_flashmessage("Invalid Operation". $this->session->userdata('is_valid_request'), 1);
        //  redirect(URL_CART_INDEX);
