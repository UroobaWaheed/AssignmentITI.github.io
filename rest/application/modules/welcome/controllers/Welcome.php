...
class Welcome extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $m=0;
        $menus=array();
        $records = $this->base_model->get_query_result("select menu_id,menu_name,description,menu_image_name from ".TBL_PREFIX.TBL_MENU." where status='Active' order by menu_id desc");
    ...
    function join_us()
    {
        ...
        $this->data['record']        = $record;
        $this->data['activemenu']     = "join_us";
        $this->data['message']         = $this->session->flashdata('message');
        $this->data['pagetitle']     = get_languageword('join_us');
        $this->data['content']         = 'join_us';
    ...
    function career()
    {
        ...
        $positions = $this->base_model->fetch_records_from('positions', array('status'=>'1'));
        $posts = array(''=>'Select position');
        foreach($positions as $ps)
        {
            $posts[$ps->title] = $ps->title;
        }
        $this->data['from_db_drop'] =$posts;/* [
            '' => 'Position Apply For',
            'Next Gen Leadership Program'  => 'Next Gen Leadership Program',
            'Area Cotch'    => 'Area Cotch',
        ...
        $this->data['record']        = $record;
        $this->data['activemenu']     = "home";
        $this->data['pagetitle']     = get_languageword('career');
        $this->data['content']         = 'career_form';
        $this->_render_page(getTemplate(), $this->data);
    }
    function feedback()
    {
        ...
        $this->data['record']        = $record;
        $this->data['activemenu']     = "home";
        $this->data['pagetitle']     = get_languageword('Feedback');
