...
class Reports extends MY_Controller
{
    ...
    function index()
    {
        if (isset($_POST['datewise_reports'])) {
            ...
            if ($from_date != '' && $to_date != '') {
                ...
                $query = "select o.customer_name,o.phone,o.no_of_items,o.total_cost,o.order_date,o.order_time,o.payment_type,o.payment_gateway from " . TBL_PREFIX . TBL_ORDERS . " o where o.status='delivered' and o.order_date and o.order_date between '" . $from_date . "' and '" . $to_date . "' ";
    ...
    function customer_wise_reports()
    {
        if (isset($_POST['client_wise_reports'])) {
            ...
            if ($user_id > 0) {
                ...
                $query = "select o.no_of_items,o.total_cost,o.order_date,o.order_time,o.payment_type,o.payment_gateway from " . TBL_PREFIX . TBL_ORDERS . " o where o.status='delivered' and o.user_id=" . $user_id . " ";
    ...
    function delivery_boy_wise_reports()
    {
        if (isset($_POST['client_wise_reports'])) {
            ...
            if ($user_id > 0) {
                ...
                $query = "select o.no_of_items,o.total_cost,o.order_date,o.order_time,o.payment_type,o.payment_gateway,o.status from " . TBL_PREFIX . TBL_ORDERS . " o where  o.dm_id=" . $user_id . " ORDER By o.order_date DESC,o.order_time DESC";

                $records = $this->db->query($query)->result();

                if (!empty($records)) {

                    $profit = "select SUM(o.paid_amount) as total_profit from " . TBL_PREFIX . TBL_ORDERS . " o where  o.dm_id=" . $user_id . " ";
                    $profit = $this->db->query($profit)->result();
         ...
         **/
        $customers = array();
        $query = "select u.id,CONCAT(u.first_name,' ',u.last_name) as customer_name from " . TBL_PREFIX . TBL_USERS . " u inner join " . TBL_PREFIX . TBL_USERS_GROUPS . " ug on u.id=ug.user_id where u.active=1 and ug.group_id=4 ";
        $users = $this->db->query($query)->result();
        ...
        if (!empty($users)) {
            ...
            $customers = array('' => get_languageword('no_delivery_boy_available'));
        }
        $this->data['customers']    = $customers;
        ...
        $this->data['activemenu']     = "reports";
        $this->data['actv_submenu'] = 'dm_wise';

        $this->data['message']         = $this->session->flashdata('message');
        $this->data['pagetitle']     = get_languageword('delivery_boy_wise_reports');
        $this->data['content']         = 'dm_wise_reports';
        $this->_render_page(TEMPLATE_ADMIN, $this->data);
    }
    ...
    function location_wise_reports()
    {
        if (isset($_POST['location_wise_reports'])) {
            ...
            if ($pincode > 0) {
                ...
                $query = "select o.customer_name,o.phone,o.no_of_items,o.total_cost,o.order_date,o.order_time,o.payment_type,o.payment_gateway from " . TBL_PREFIX . TBL_ORDERS . " o where o.status='delivered' and o.pincode=" . $pincode . " ";
    ...
    function item_wise_reports()
    {
        if (isset($_POST['item_wise_reports'])) {
            ...
            if ($item_id > 0) {
                ...
                $query = "select o.customer_name,o.phone,o.no_of_items,o.total_cost,o.order_date,o.order_time,o.payment_type,o.payment_gateway,r.item_cost from " . TBL_PREFIX . TBL_ORDERS . " o inner join " . TBL_PREFIX . TBL_ORDER_PRODUCTS . " r on o.order_id=r.order_id where o.status='delivered' and r.item_id=" . $item_id . " ";
    ...
    function most_order_item_reports()
    {

        //$this->data['menus']   = prepare_dropdown(TBL_MENU, 1, 'menu_id', 'menu_name', '', array('status' => 'Active'));

        $query = "select count(r.item_id) as totalcount,r.item_name,r.item_cost from " . TBL_PREFIX . TBL_ORDER_PRODUCTS . " r where is_deleted=0 group by r.item_id ORDER BY totalcount DESC";
        ...
        $this->data['activemenu']     = "reports";
        $this->data['actv_submenu'] = 'moi_wise';

        $this->data['message']         = $this->session->flashdata('message');
        $this->data['pagetitle']     = get_languageword('most_order_item_reports');
        $this->data['content']         = 'most_orders_item';
        $this->_render_page(TEMPLATE_ADMIN, $this->data);
    }
    function job_response_reports()
    {

        //$this->data['menus']   = prepare_dropdown(TBL_MENU, 1, 'menu_id', 'menu_name', '', array('status' => 'Active'));

//        $query = "select count(c.jobapply) as totalcount,r.title from " . TBL_PREFIX  . "positions r inner join " . TBL_PREFIX  . "career c on c.jobapply=r.title    group by r.title ORDER BY totalcount DESC";
        $query = "select c.id as id, c.fullName as applicant, r.title from " . TBL_PREFIX  . "positions r inner join " . TBL_PREFIX  . "career c on c.jobapply=r.title ORDER BY id DESC";
        $records = $this->db->query($query)->result();
        ...
        $this->data['activemenu']     = "reports";
        $this->data['actv_submenu'] = 'job_wise';

        $this->data['message']         = $this->session->flashdata('message');
        $this->data['pagetitle']     = get_languageword('Job Responses');
        $this->data['content']         = 'job_response';
        $this->_render_page(TEMPLATE_ADMIN, $this->data);
    }
}
