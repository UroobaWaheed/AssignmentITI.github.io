...
class Orders extends MY_Controller
{
    ...
    protected $template = TEMPLATE_ADMIN;
    ...
    function __construct()
    {
        ...
        if (!in_array($usertype, array('admin', 'kitchen_manager'))) {
            exit;
        }
        $this->usertype = $usertype;
        if ($usertype == 'kitchen_manager') {
            $this->template = TEMPLATE_KM;
        }
    }
    ...
    {
        if ($order_type == '') {
            if ($this->usertype == 'kitchen_manager') {
                $order_type = 'new';
            } else {
                $order_type = 'all';
            }
    ...
    function view_order()
    {
        ...
        if ($order->status == 'new' || $order->status == 'process') {
            ...
            if ($order_city_id > 0) {
                ...
                $dm_users = $this->base_model->get_query_result("select id,username from " . TBL_PREFIX . TBL_USERS . " where FIND_IN_SET($order_city_id,assigned_cities) and active=1 and is_available=1");
