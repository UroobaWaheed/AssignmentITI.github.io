		...
		if ($msg != "") {
			...
			if ($CI->config->item('email_settings')->mail_config == "webmail") {
				...
				try {
					...
					$mail->CharSet = 'UTF-8';
					$mail->isSMTP();
					$mail->Host = $CI->config->item('email_settings')->smtp_host;
					$mail->SMTPAuth = true;
					$mail->Username = $CI->config->item('email_settings')->smtp_user;
					$mail->Password = $CI->config->item('email_settings')->smtp_password;
					$mail->SMTPSecure = 'ssl';
					$mail->Port = $CI->config->item('email_settings')->smtp_port;
					//	$mail->SMTPDebug = 2;



					$mail->setFrom($CI->config->item('email_settings')->smtp_user, $CI->config->item('site_settings')->site_title);
