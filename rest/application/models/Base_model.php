...
class Base_Model extends CI_Model  
{
	...
	private function _get_datatables_customquery_feed($query, $columns = array(), $order = array())
	{	
		$i = 0;
		$column = array();
		$str = '';
	...
	function get_datatables($table, $type = 'auto', $condition = array(), $columns = array(), $order = array())
	{
		...
		else if($type == 'customnew')
		{
			...
			$query = $this->db->query($query_str);
		}else if($type == 'feednew')
		{
			$query_str = $this->_get_datatables_customquery_feed($table, $columns, $order);
			//echo $query_str;die();
			$queryall = $this->db->query($query_str);
			$this->numrows = $this->db->affected_rows();
			if($_POST['length'] != -1)
