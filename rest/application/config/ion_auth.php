...
if ($hostname=='' || $username=='' || $database=='') {
	redirect(SITEURL.'install');
} 

//Site Settings
$site_settings = $CI->db->get('site_settings')->row();
if(!empty($site_settings)) {
	$CI->config->set_item('site_settings', $site_settings);
}

//Social Networks
$social_networks = $CI->db->get('social_networks')->row();
if(!empty($social_networks)) {
	$CI->config->set_item('social_networks', $social_networks);
}
...
if(!empty($email_settings)) {
	$CI->config->set_item('email_settings', $email_settings);
}

//Paypal Settings
$paypal_settings = $CI->db->get('paypal_settings')->row();
if(!empty($paypal_settings)) {
	$CI->config->set_item('paypal_settings', $paypal_settings);
}

//Point Settings
$point_settings = $CI->db->get('loyality_points')->row();
if(!empty($point_settings)) {
	$CI->config->set_item('point_settings', $point_settings);
}

//Point Settings
$referral_settings = $CI->db->get('referral_settings')->row();
if(!empty($referral_settings)) {
	$CI->config->set_item('referral_settings', $referral_settings);
}



//Tinify Settings
$tinify_settings = $CI->db->get('tinify_settings')->row();
if(!empty($tinify_settings)) {
	$CI->config->set_item('tinify_settings', $tinify_settings);
}


//SEO Settings
$seo_settings = $CI->db->get('seo_settings')->row();
if(!empty($seo_settings)) {
	$CI->config->set_item('seo_settings', $seo_settings);
}

$config['site_title']                = (isset($CI->config->item('site_settings')->site_title)) ? $CI->config->item('site_settings')->site_title : '';      // Site Title, example.com
$config['admin_email']               = ''; 	// Admin Email, admin@example.com

$config['default_group']             = 'user';           // Default group, use name
$config['member_group']              = 'user';           // Default group, use name
$config['admin_group']               = 'admin';          // Default administrators group, use name

$config['kitchen_manager_group']     = 'kitchen_manager';          // Default administrators group, use name

$config['delivery_manager_group']    = 'delivery_manager';          // Default administrators group, use name
